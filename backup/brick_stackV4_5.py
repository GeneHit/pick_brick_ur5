...
class brick_stack:
    ...
    def Compare_twoPosition(self,pos1,pos2):
        if math.fabs(pos1.position.x-pos2.position.x)<0.001 and \
           math.fabs(pos1.position.y-pos2.position.y)<0.001 and \
           math.fabs(pos1.position.z-pos2.position.z)<0.001 :
            return True
        else:
            return False 
    ...
    while not rospy.is_shutdown():
        ...
        #**3***********************plan to the place pose********************** 
            ...
            if arrived_flag == 0:
                ...
                current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
                waypoints=sys.portal_plan(current_pose,sys.place_pose,0.05,0.8)
