...
author: YunkeYu
function:
****test the the move of UR5's end using compute_cartesian_path() to pick and place
"""
...
def portal_plan(pose1,pose2,t1,t2):
    ...
    portal2.position.z=pose1.position.z+t2*(pose2.position.z-pose1.position.z)+0.1
    portal_pose = [ ]
    ...
    while not rospy.is_shutdown():
        ...
        #**1*****plan to the brick pose above 0.3m to search brick again*******    
            ...
            if arrived_flag is False:
                ...
                if time_cal == 50 :
                    ...
                    arrived_flag = False
                current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
                if math.fabs(current_pose.position.x- \
                                target.position.x)<0.001 and \
                   math.fabs(current_pose.position.y- \
                                target.position.y)<0.001 and \
                   math.fabs(current_pose.position.z- \
        ...
        #**3***********************plan to the place pose********************** 
            ...
            if arrived_flag is False:
                ...
                current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
                waypoints=portal_plan(current_pose,sys.place_pose,0.05,0.8)
        ...
        #**5****************up 0.3m  and go to the innitial pose*******************
            ...
            if arrived_flag is False:
                ...
                current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
                if \
                math.fabs(current_pose.position.x-target.position.x)<0.001 and \
                math.fabs(current_pose.position.y-target.position.y)<0.001 and \
                math.fabs(current_pose.position.z-target.position.z)<0.001 :
                    sysmode=0
                    arrived_flag = False
