...
class brick_stack:
    def __init__(self):
        ...
        #self.target = deepcopy(self.initial_pose)
    ...
    def go_cartesian_pose(self,wayposes,single_pose_flag):
        if single_pose_flag == 1:
            waypoints = [ ]
    ...
    def get_place_pose(self,place_num):
        ...
        brick_length=0.24+0.005
        brick_width=0.115+0.005
        brick_height=0.053
        #position of the center of the stacking bricks
        center_x = -0.6
        center_y = 0.0
...
def main(args):
    ...
    go_flag=sys.go_cartesian_pose( \
                            sys.search_pose[sys.search_load[0]],1)
    ...
    while not rospy.is_shutdown():
        ...
        if sysmode == 0 :   #search the brick
            rospy.loginfo("in 0")
            #judge that does it arrive the target
            if first_time == True:
                arrived_flag = True
                first_time = False
            current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
            if math.fabs(current_pose.position.x-target.position.x)<0.001 and \
               math.fabs(current_pose.position.y-target.position.y)<0.001 and \
               math.fabs(current_pose.position.z-target.position.z)<0.001 :
                arrived_flag = True
            if arrived_flag == True :
                ...
                if sys.brick_flag is True:
                    ...
                    arrived_flag = True
                    target = sys.arm.get_current_pose(sys.end_effector_link).pose
                else:
                    go_flag=sys.go_cartesian_pose( \
                            sys.search_pose[sys.search_load[search_flag]],1)
                    if go_flag == True:
                        target = sys.search_pose[sys.search_load[search_flag]]
                        search_flag += 1
            #publish sysymode=0,so that trigge the img process
            sys.sysmode_pub.publish(str(sysmode))
            sleep(0.02)
        #**0-endle*******************search the brick**************************
        #**1******************take any ways to arrive the object***************
        elif sysmode == 1:  
            rospy.loginfo("in 1")
            #judge that does it arrive the target
            current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
            ...
            if arrived_flag == True :
                ...
                if go_flag is True:
                    ...
                    sear2obj_flag += 1
                    if go_flag is True:
                        target = sys.search_pose[sys.search_load[sear2obj_flag]]
                else:
                    sysmode = 0
                    sear2obj_flag = 0
        #**1-endle***********take any ways to arrive the object***************
        #**2**************arrived the brick and pick the brick****************
        elif sysmode == 2:  #picks the brick
            rospy.loginfo("in 2")
            #judge that does it arrive the target
            current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
            if math.fabs(current_pose.position.x-target.position.x)<0.001 and \
               ...
               math.fabs(current_pose.position.z-target.position.z)<0.001 :
                arrived_flag = True
            #if arrived the brick,open the vacuum pump
            if arrived_flag == True:
                arrived_flag = False
        ...
        #**3***********************plan to the place pose********************** 
            ...
            #current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
            #if math.fabs(current_pose.position.x-sys.place_pose.position.x)<0.05 and \
            #    math.fabs(current_pose.position.y-sys.place_pose.position.y)<0.05 and \
            #    math.fabs(current_pose.position.z-sys.place_pose.position.z)<0.05 :
            if go_flag == True:
                target = sys.place_pose
                sysmode = 4
        ...
        #**4***********arrived the place pose and place the brick**************
            ...
            if math.fabs(current_pose.position.x-target.position.x)<0.001 and \
               ...
               math.fabs(current_pose.position.z-target.position.z)<0.001 :
                isComplete=sys.pump_contr(True,current_pose,True)
                arrived_flag = True
            #if arrived the brick,close the vacuum pump
            if arrived_flag == True:
                arrived_flag = False
                isComplete=sys.pump_contr(False,current_pose,False)
                #isComplete=sys.pump_contr(False)
                #**4.1-******up 0.3m from place current pose and go to the innitial pose
                current_pose.position.z = current_pose.position.z+0.1
                waypoints=[ ]
                waypoints.append(deepcopy(current_pose))
                waypoints.append(deepcopy(sys.search_pose[search_flag]))
                go_flag=sys.go_cartesian_pose(waypoints,2)
                #**4.1*******endle
                if go_flag is True:
                    target = sys.search_pose[search_flag]
                    sysmode=0
                    sys.brick_flag=False
        #**4-endle*********arrived the place pose and place the brick**************
        #**5********************waiting for arriving the target********************
        elif sysmode == 5:
            current_pose = sys.arm.get_current_pose(sys.end_effector_link).pose
            if math.fabs(current_pose.position.x-target.position.x)<0.001 and \
               math.fabs(current_pose.position.y-target.position.y)<0.001 and \
               math.fabs(current_pose.position.z-target.position.z)<0.001 :
                pass
        #**5-endle****************waiting for arriving the target******************
        #**6******nothing to do********
        else:
            pass
        #**6-endle*********************
