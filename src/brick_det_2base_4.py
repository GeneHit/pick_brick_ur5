...
class brick_detection:
    def __init__(self):
        self.image_pub = rospy.Publisher("brick_position", Tracker,queue_size=1)
    ...
    def callback(self, data):
        ...
        if self.det_bool is True:
            ...
            try:
                ...
                if brick_base[0] == 1:
                    self.Tracker.isDetected = True
                    self.Tracker.position_x =brick_base[1]
                    self.Tracker.position_y =brick_base[2]   
                    self.Tracker.position_z =brick_base[3]
                    self.Tracker.orientation_x =brick_base[4]
                    self.Tracker.orientation_y =brick_base[5]
                    self.Tracker.orientation_z =brick_base[6]
                    self.Tracker.orientation_w =brick_base[7]
                else:
                    self.Tracker.isDetected = False
                self.image_pub.publish(self.Tracker)
            except CvBridgeError as e:
                rospy.loginfo(e)
